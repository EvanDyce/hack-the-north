...
if not os.path.isdir(full_path):
    ...
    video_files = [
        f"{full_path}/{f}" for f in os.listdir(full_path) if f.lower().endswith("mp4")
    ]
    excel_files = [
        f"{full_path}/{f}"
        for f in os.listdir(full_path)
    ...
    csv_files = [
        f"{full_path}/{f}" for f in os.listdir(full_path) if f.lower().endswith("csv")
    ]
